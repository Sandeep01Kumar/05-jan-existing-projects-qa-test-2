...
- [Add Firebase to your Flutter app](https://firebase.google.com/docs/flutter/setup) — Canonical official Firebase guidance for FlutterFire plugin adoption (including the `flutterfire configure` CLI flow), the platform-specific `Firebase.initializeApp()` initialization, and the supported Flutter / Dart version envelopes that constrain the FlutterFire-version-compatibility considerations referenced in this epic. Specific Fabric-to-FlutterFire migration narrative and the literal `FlutterError.onError` binding pattern are `[inferred — no direct source]` because no equivalent dedicated Fabric → FlutterFire migration page is published by Firebase; FlutterFire documents the post-migration patterns directly on the Crashlytics usage and overview pages.
