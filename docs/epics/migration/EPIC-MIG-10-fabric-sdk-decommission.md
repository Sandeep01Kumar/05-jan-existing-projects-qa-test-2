...
- [Get started with Firebase Crashlytics for Android — Migrate from a legacy Crashlytics SDK](https://firebase.google.com/docs/crashlytics/get-started?platform=android) — The canonical Firebase documentation that absorbed the retired `upgrade-from-fabric?platform=android` migration page; grounds the AndroidX prerequisite that frames the broader Android migration referenced in `STORY-MIG-10-S01`. The specific Android-cleanup directives — removal of the Fabric Maven repository declaration from the root-level `build.gradle`, removal of the Fabric Gradle plugin classpath (`io.fabric:io.fabric.gradle.plugin`) from `buildscript.dependencies`, and removal of the `apply plugin: 'io.fabric'` line from module-level `build.gradle` — are `[inferred — no direct source]` because Firebase has streamlined its legacy-removal copy now that the November 15, 2020 sunset has passed; these are the well-known historical Fabric-cleanup operations preserved here to ensure complete decommission coverage.
