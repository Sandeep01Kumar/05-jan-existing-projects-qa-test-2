...
- [Get started with Firebase Crashlytics for Android — Migrate from a legacy Crashlytics SDK](https://firebase.google.com/docs/crashlytics/get-started?platform=android) — The canonical Firebase documentation that absorbed the retired `upgrade-from-fabric?platform=android` migration page; documents the Android migration steps including the removal of the Fabric Crashlytics SDK dependency, the AndroidX prerequisite (the Firebase Crashlytics SDK uses AndroidX as a dependency), and the addition of the Firebase Crashlytics Gradle plugin classpath in place of the legacy Fabric Gradle plugin. The literal Fabric Maven repository URL (`maven { url 'https://maven.fabric.io/public' }`) and the literal `io.fabric.tools:gradle` classpath removal directives are `[inferred — no direct source]` from this present-day page because Firebase has streamlined the legacy-removal copy now that the November 15, 2020 sunset has passed.
