...
- [Add Firebase to your Android project](https://firebase.google.com/docs/android/setup) — AndroidX prerequisite for Firebase Android SDKs (including Firebase Crashlytics) referenced by `EPIC-MIG-03`; the Fabric Maven repository and Fabric Gradle plugin classpath cleanup details are `[inferred — no direct source]` because the dedicated `upgrade-sdk?platform=android` migration page has been retired and redirected to the general get-started guide.
